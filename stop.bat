...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$ports=@(8000,5173); foreach($p in $ports){ try { $procs = Get-NetTCPConnection -LocalPort $p -State Listen -ErrorAction SilentlyContinue | Select-Object -ExpandProperty OwningProcess -Unique; foreach($pid in $procs){ try { $proc = Get-Process -Id $pid -ErrorAction Stop; Write-Host ('Port ' + $p + ' (PID ' + $pid + ', ' + $proc.ProcessName + ') -> kill'); Stop-Process -Id $pid -Force -ErrorAction SilentlyContinue } catch {} } } catch {} }"
