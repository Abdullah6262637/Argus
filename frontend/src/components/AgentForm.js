...
export function AgentForm({ initial, onSubmit, onCancel, submitting = false }) {
    ...
    const modelSuggestions = useMemo(() => {
        const fromCatalog = catalog ? catalog[provider] : null;
    ...
    useEffect(() => {
        if (!isEditing)
            setModel(modelSuggestions[0]?.id ?? '');
    ...
    const handleSubmit = async () => {
        ...
        const cleanMedia = (m) => {
            if (!m.enabled && !m.provider && !m.model && !m.base_url && !m.api_key)
                return null;
            return {
                enabled: !!m.enabled,
    ...
    return (_jsx("div", { onClick: (e) => {
        ...
        }, className: `fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 ${isClosing ? 'animate-backdrop-out' : 'animate-backdrop-in'}`, children: _jsxs("div", { className: `w-full max-w-3xl max-h-[92vh] flex flex-col rounded-lg border border-brand-borderStrong bg-brand-panel shadow-2xl relative overflow-hidden ${isClosing ? 'animate-modal-out' : 'animate-modal-in'}`, children: [_jsxs("div", { className: "absolute inset-0 pointer-events-none z-0", children: [_jsx("div", { className: `absolute -top-40 -left-40 w-96 h-96 rounded-full bg-gradient-to-br ${STEP_GLOWS[step]} blur-[80px] transition-all duration-700 ease-out` }), _jsx("div", { className: `absolute -bottom-40 -right-40 w-96 h-96 rounded-full bg-gradient-to-br ${STEP_GLOWS[step]} blur-[80px] transition-all duration-700 ease-out` })] }), _jsxs("div", { className: "relative z-10 px-6 py-4 border-b border-brand-border bg-brand-panelAlt/90 backdrop-blur-[2px]", children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("div", { children: [_jsx("h2", { className: "text-base font-semibold text-brand-text tracking-wide", children: isEditing ? 'Ajani Duzenle' : 'Yeni Ajan Olustur' }), _jsxs("p", { className: "text-[11px] text-brand-muted mt-0.5", children: ["Adim ", step + 1, " / ", STEPS.length, " \u2014 ", STEPS[step]] })] }), _jsx("button", { onClick: handleClose, className: "text-brand-muted hover:text-brand-text text-2xl leading-none px-2", "aria-label": "Kapat", children: "\u00D7" })] }), _jsx(Stepper, { current: step, onJump: (i) => canJumpTo(i) && setStep(i), canJumpTo: canJumpTo })] }), _jsxs("div", { className: "relative z-10 h-[480px] overflow-y-auto p-6 border-b border-brand-border/60", children: [_jsxs("div", { className: "animate-step-in", children: [step === 0 && (_jsx(Step1Basic, { name: name, setName: setName, role: role, setRole: setRole, description: description, setDescription: setDescription, onApplyTemplate: applyTemplate, selectedTemplateId: selectedTemplateId, onClearTemplate: handleClearTemplate, isEditing: isEditing })), step === 1 && (_jsx(Step2LLM, { provider: provider, setProvider: setProvider, model: model, setModel: setModel, baseUrl: baseUrl, setBaseUrl: setBaseUrl, apiKey: apiKey, setApiKey: setApiKey, showApiKey: showApiKey, setShowApiKey: setShowApiKey, clearApiKey: clearApiKey, setClearApiKey: setClearApiKey, presetId: presetId, applyPreset: applyPreset, modelSuggestions: modelSuggestions, isEditing: isEditing, initial: initial, onTest: handleTest, testing: testing, testResult: testResult, envStatus: envStatus, verifySsl: verifySsl, setVerifySsl: setVerifySsl })), step === 2 && (_jsx(Step3Media, { image: image, setImage: setImage, video: video, setVideo: setVideo, audio: audio, setAudio: setAudio, isEditing: isEditing, initialImageMask: initial?.image?.api_key_masked, initialVideoMask: initial?.video?.api_key_masked, initialAudioMask: initial?.audio?.api_key_masked })), step === 3 && (_jsx(StepBehavior, { systemPrompt: systemPrompt, setSystemPrompt: setSystemPrompt, temperature: temperature, setTemperature: setTemperature, maxTokens: maxTokens, setMaxTokens: setMaxTokens, tagsText: tagsText, setTagsText: setTagsText, isActive: isActive, setIsActive: setIsActive })), step === 4 && (_jsx(Step5Permissions, { permissions: permissions, setPermissions: setPermissions })), step === 5 && _jsx(Step6Plugins, {})] }, step), error && (_jsx("div", { className: "mt-4 p-3 text-xs text-brand-danger bg-brand-danger/10 border border-brand-danger/40 rounded", children: error }))] }), _jsxs("div", { className: "relative z-10 flex items-center justify-between gap-2 px-6 py-4 bg-brand-panelAlt/90 backdrop-blur-[2px]", children: [_jsx("button", { onClick: handleClose, className: "px-4 py-2 text-sm rounded border border-brand-border text-brand-textSoft hover:text-brand-text hover:border-brand-borderStrong transition", children: "Iptal" }), _jsxs("div", { className: "flex items-center gap-2", children: [_jsx("button", { onClick: () => setStep((s) => Math.max(0, s - 1)), disabled: step === 0, className: "px-4 py-2 text-sm rounded border border-brand-border text-brand-textSoft hover:text-brand-text disabled:opacity-30 transition", children: "Geri" }), step < STEPS.length - 1 ? (_jsx("button", { onClick: () => canGoNext() && setStep((s) => (s + 1)), disabled: !canGoNext(), className: "px-5 py-2 text-sm rounded bg-brand-accent text-brand-bg font-semibold hover:bg-brand-accentDim disabled:opacity-40 transition", children: "Ileri" })) : (_jsx("button", { onClick: handleSubmit, disabled: submitting, className: "px-5 py-2 text-sm rounded bg-brand-accent text-brand-bg font-semibold hover:bg-brand-accentDim disabled:opacity-50 transition", children: submitting
                                        ? 'Kaydediliyor...'
                                        : isEditing
                                            ? 'Degisiklikleri Kaydet'
                                            : 'Ajani Olustur' }))] })] })] }) }));
}
function Stepper({ current, onJump, canJumpTo }) {
    return (_jsx("div", { className: "flex items-center gap-1 mt-3", children: STEPS.map((label, i) => {
            const active = i === current;
            const done = i < current;
            const allowed = canJumpTo(i);
            return (_jsxs("button", { type: "button", onClick: () => allowed && onJump(i), disabled: !allowed, className: `flex-1 group transition-all duration-200 ${!allowed ? 'opacity-40 cursor-not-allowed' : ''}`, children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx("div", { className: `w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold border transition ${active
                                    ? 'bg-brand-accent text-brand-bg border-brand-accent'
                                    : done
                                        ? 'bg-brand-accent/20 text-brand-accent border-brand-accent/40'
                                        : 'bg-brand-bg text-brand-muted border-brand-border'}`, children: done ? _jsx(Icon, { name: "check", size: 14, weight: 700 }) : i + 1 }), _jsx("div", { className: `text-[10px] uppercase tracking-wider font-semibold hidden md:block ${active ? 'text-brand-accent' : 'text-brand-muted'}`, children: label })] }), i < STEPS.length - 1 && (_jsx("div", { className: `h-px mt-3 ml-6 ${done ? 'bg-brand-accent/50' : 'bg-brand-border'}` }))] }, label));
        }) }));
}
